...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action == 'accept') {
        ...
        header("Location: proses_purchasing.php?action=accept&id=".$id_pemesanan);
        exit();
    } elseif ($action == 'decline') {
        // Update status to declined
        $update = $mysqli->prepare("UPDATE pemesanan SET status = 2 WHERE id_pemesanan = ?");
        $update->bind_param("i", $id_pemesanan);
        $update->execute();
        
        $_SESSION['message'] = "Order #{$id_pemesanan} has been declined";
