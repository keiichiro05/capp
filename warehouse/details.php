...
if (isset($_GET['download']) && $_GET['download'] == 'pdf') {
    ...
    if (mysqli_num_rows($warehouse_history) > 0) {
        ...
        mysqli_data_seek($warehouse_history, 0); // Reset pointer to beginning
        
        while ($warehouse = mysqli_fetch_assoc($warehouse_history)) {
            $html .= '<tr>
                <td>' . htmlspecialchars($warehouse['movement_date']) . '</td>
                <td>' . htmlspecialchars($warehouse['movement_type']) . '</td>
                <td>' . htmlspecialchars($warehouse['new_stock']) . '</td>
                <td>' . htmlspecialchars($warehouse['previous_stock']) . '</td>
                <td>' . htmlspecialchars($warehouse['pic']) . '</td>
                <td>' . htmlspecialchars($warehouse['warehouse']) . '</td>
            </tr>';
        }
