...
if (isset($_GET['export'])) {
    ...
    $headers = array('Date', 'Type', 'Product Code', 'Product Name', 'Quantity', 'Previous Stock', 'New Stock', 'Difference', 'Warehouse', 'PIC', 'Notes');
    $rows = array();
                                            ...
                                            <tbody>
                                                ...
                                                if ($movement_result && $movement_result->num_rows > 0) {
                                                    while ($movement = $movement_result->fetch_assoc()) {
                                                        $type_class = $movement['movement_type'] == 'inbound' ? 'label-success' : 'label-danger';
                                        ...
                                        <ul class="pagination">
                                            ...
                                            for ($i = 1; $i <= $total_pages; $i++) {
                                                echo "<li" . ($current_page == $i ? " class='active'" : "") . ">
                                                    <a href='movement_history.php?submenu=movement-history&cabang=" . urlencode($cabang_filter) . "&type=" . urlencode($movement_type) . "&page=$i'>$i</a>
                                                </li>";
                                            }
                                            ?>
                                        </ul>
