...
if (isset($_POST['outbound'])) {
    ...
    if (empty($id_barang) || empty($jumlah) || empty($tujuan)) {
        header("location:stock.php?tab=outbound&status=error&message=All required fields must be filled.");
        exit();
    }

    if ($jumlah <= 0) {
        header("location:stock.php?tab=outbound&status=error&message=Quantity must be greater than zero.");
        exit();
    }
    ...
    try {
        ...
        // 4. Record the outbound transaction in outbound_log
        $tanggal = date('Y-m-d H:i:s');
        $stmt_insert_log = $mysqli->prepare("INSERT INTO outbound_log (id_barang, nama_barang, jumlah, tanggal, id_pegawai, tujuan, keterangan) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        if (!$stmt_insert_log->execute()) {
            throw new Exception("Error recording outbound log: " . $stmt_insert_log->error);
        }
        $stmt_insert_log->close();

        // Commit the transaction
        $mysqli->commit();
        header("location:stock.php?tab=outbound&status=success&message=Outbound recorded successfully!");
        exit();

    } catch (Exception $e) {
        // Rollback the transaction on error
        $mysqli->rollback();
        header("location:stock.php?tab=outbound&status=error&message=" . urlencode($e->getMessage()));
        exit();
    }
} else {
